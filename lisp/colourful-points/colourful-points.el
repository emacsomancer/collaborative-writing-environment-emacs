...
;; based on https://github.com/jrockway/elisp/blob/master/_local/colorful-points.el
;; by Jonathan Rockway ( https://github.com/jrockway ) and Kevin Brubeck Unhammer ( https://github.com/unhammer )
...
(defun colorful-points--next-color ()
  ...
  (let ((colors colorful-points-colors))
    (cl-loop for color in
          (mapcar (lambda (cell) (cl-getf (overlay-get (cdr cell) 'face) :background)) colorful-points)
  ...
  (cl-loop for (point . overlay) in
        (mapcar (lambda (win) (cons (window-point win) (colorful-points--overlay-for-window win)))
                (cl-remove-if-not (lambda (window)
                               ...
                               (cl-loop for frame in (frame-list) nconcing (window-list frame))))
        do (let ((first point)
                 (last (1+ point))
